...
import type { SellerStore, Product } from '@/lib/types/v2'
...
export default async function StorePage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* 배너 */}
      <div className="relative h-48 sm:h-64 w-full overflow-hidden" style={{ backgroundColor: themeColor + '20' }}>
      ...
      <div className="max-w-5xl mx-auto px-4">
        <div className="relative -mt-10 mb-8 flex items-end gap-5">
          <div className="w-20 h-20 rounded-2xl border-4 border-white shadow-lg bg-white overflow-hidden flex-shrink-0">
            {store.logo_url ? <img src={store.logo_url} alt="logo" className="w-full h-full object-cover" /> : (
              <div className="w-full h-full flex items-center justify-center text-3xl" style={{ backgroundColor: themeColor + '20' }}>🏪</div>
            )}
          </div>
          <div className="pb-2">
          ...
          {!products || products.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {products.map((p: Product) => (
                <Link key={p.id} href={`/stores/${params.slug}/products/${p.id}`}
                  ...
                  <div className="p-3">
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      ...
                      {p.compare_price && (
                        <span className="text-xs text-gray-400 line-through">{p.compare_price.toLocaleString()}원</span>
                      )}
                    </div>
                    {p.status === 'sold_out' && (
                      <span className="inline-block mt-1.5 text-xs px-2 py-0.5 bg-gray-100 text-gray-500 rounded-full">품절</span>
                    )}
                  </div>
                </Link>
              ))}
