...
export default function AdminSellersPage() {
  ...
  const handleApprove = async () => {
    ...
    else if (!data.success) alert('오류: ' + data.error)
    else {
      alert(`승인 완료! 소호몰 URL: /stores/${data.slug}`)
      setSelected(null); setAdminNote(''); fetchApplications()
    }
    setProcessing(false)
  }

  const handleReject = async () => {
    if (!selected || !adminNote.trim()) { alert('거절 사유를 입력해 주세요.'); return }
    setProcessing(true)
    ...
    else { setSelected(null); setAdminNote(''); fetchApplications() }
    setProcessing(false)
  }

  const counts = { pending: 0, approved: 0, rejected: 0 }
  // counts는 별도 API에서 가져오거나 DB aggregate로 처리
          ...
          <div className="flex gap-1 mt-4 bg-gray-100 rounded-lg p-1">
            {(['pending', 'approved', 'rejected'] as Filter[]).map(f => (
              <button key={f} onClick={() => { setFilter(f); setSelected(null) }}
                ...
                {f === 'pending' ? '대기 중' : f === 'approved' ? '승인됨' : '거절됨'}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-32 text-gray-400">
              <p className="text-sm">신청서가 없습니다</p>
            </div>
          ) : applications.map(app => (
            <button key={app.id} onClick={() => { setSelected(app); setAdminNote('') }}
              className={`w-full text-left p-4 rounded-xl border transition-all
                ${selected?.id === app.id ? 'border-indigo-300 bg-indigo-50' : 'border-gray-100 bg-white hover:border-gray-200 hover:bg-gray-50'}`}>
        ...
        {!selected ? (
          ...
          <div className="max-w-2xl mx-auto p-8">
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-2xl font-bold text-gray-900">{selected.store_name}</h2>
                <p className="text-gray-500 text-sm mt-1">{selected.store_category} · 신청일 {new Date(selected.created_at).toLocaleDateString('ko-KR')}</p>
              </div>
              <span className={`px-3 py-1 rounded-full text-sm font-medium
                ${selected.status === 'pending' ? 'bg-amber-100 text-amber-700'
                  : selected.status === 'approved' ? 'bg-green-100 text-green-700'
                  : 'bg-red-100 text-red-700'}`}>
                {selected.status === 'pending' ? '검토 대기' : selected.status === 'approved' ? '승인됨' : '거절됨'}
              </span>
            </div>
            ...
            <Section title="사업자 정보">
              <InfoGrid items={[
                ...
                { label: '사업자 유형', value: selected.business_type === 'individual' ? '개인사업자' : '법인사업자' },
                { label: '대표자', value: selected.representative },
                { label: '사업자번호', value: selected.business_number || '-' },
            ...
            {/* 소호몰 정보 */}
            <Section title="소호몰 정보">
              <InfoGrid items={[
                { label: '상점명', value: selected.store_name },
                { label: '예정 URL', value: `/stores/${selected.store_slug}` },
                { label: '카테고리', value: selected.store_category },
              ]} />
              {selected.store_intro && (
            ...
            {selected.status === 'pending' && (
              <Section title="검토 및 처리">
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">
                      관리자 메모
                      <span className="ml-1 text-xs text-gray-400 font-normal">(거절 시 필수 — 신청자에게 전달됩니다)</span>
                    </label>
                    <textarea value={adminNote} onChange={e => setAdminNote(e.target.value)}
                      rows={3} placeholder="승인 시: 추가 안내 메모 (선택) | 거절 시: 거절 사유 입력"
                  ...
                  <div className="flex gap-3">
                    <button onClick={handleReject} disabled={processing}
                      className="flex-1 py-3 border-2 border-red-200 text-red-600 rounded-xl font-medium hover:bg-red-50 disabled:opacity-60 transition-colors">
                      {processing ? '처리 중...' : '거절'}
                    </button>
                    <button onClick={handleApprove} disabled={processing}
                      className="flex-2 px-8 py-3 bg-indigo-600 text-white rounded-xl font-medium hover:bg-indigo-700 disabled:opacity-60 transition-colors">
            ...
            {selected.status !== 'pending' && selected.admin_note && (
              <Section title="처리 메모">
                <p className="text-sm text-gray-700 bg-gray-50 rounded-lg p-3">{selected.admin_note}</p>
                <p className="text-xs text-gray-400 mt-2">
