...
export default function CategoriesPage() {
  ...
  const [form, setForm] = useState({ name: '', description: '', sort_order: 0, level: 1 as Level, parent_id: null as string | null })
  ...
  function openModal(level: Level, parent_id: string | null) {
  ...
  return (
    <>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={`${levelLabel(form.level)} 추가`} size="sm">
        <FormField label="분류명" required>
          <Input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="분류명 입력" autoFocus />
        </FormField>
        ...
        <FormField label="설명 (선택)" className="mt-3">
          <Input value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="선택사항" />
        </FormField>
...
function CatRow({ name, level, onAdd, onDelete, addLabel }: {
  name: string; level: 1 | 2 | 3
  onAdd?: () => void; onDelete: () => void; addLabel?: string
}) {
  const icon = level === 1 ? '📁' : level === 2 ? '📂' : '📄'
  const indent = level === 1 ? '' : level === 2 ? 'border-l-2 border-border pl-3' : 'border-l-2 border-border pl-3'
  return (
